...
import com.example.acsha.ellipsizetextview.R;
 ...
 * @author Vince
 */
public class CustomATextView extends View {
    public final static String NEW_LINE_STR = "\n";

    private int mLineSpacing;

    // the inform listener when measure is done
    public CustomATextView(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.CustomATextView);

        mStrEllipsis = "...";
        mMaxLineCount = typeArray.getInteger(R.styleable.CustomATextView_maxLines, 5);
        mLineSpacing = typeArray.getDimensionPixelSize(R.styleable.CustomATextView_lineSpacing, 0);

        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(typeArray.getColor(R.styleable.CustomATextView_textColor, Color.BLACK));
        mPaint.setTextSize(typeArray.getDimensionPixelSize(R.styleable.CustomATextView_textSize, 0));
